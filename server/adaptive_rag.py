...
from langchain_chroma import Chroma  # ✅ NEW Chroma
...
# Initialize ChromaDB client (this replaces your old PathwayVectorClient)
client = vector_store
...
def retrieve(state):
    ...
    metadata_filter = {
        # "path": {"$regex": f".*{state['company_name']}_{state['year']}.*"}
    }
    ...
    if queries[0] != "" and count == 1:
        for query in queries:
            ...
            # try:
            table_docs = client.similarity_search_with_score(
                table_query, 
                k=10, 
            ...
            for doc, score in table_docs:
                ...
                print(f"Table content found: {doc.page_content[:100]}... with score {score}")
            # except Exception as e:
                #print(f"Table search error: {e}")
                # Fallback without metadata filter
                # table_docs = client.similarity_search_with_score(table_query, k=10)
                # for doc, score in table_docs:
                #     if doc.metadata.get("category") == "Table":
                #         table_results.append((doc, score))
            ...
            try:
                text_docs = client.similarity_search_with_score(
                    query, 
                    k=10, 
                    # filter=metadata_filter
                )
                text_results.extend(text_docs)
            except Exception as e:
                # print(f"Text search error: {e}")
                # Fallback without metadata filter
                text_docs = client.similarity_search_with_score(query, k=10)
                text_results.extend(text_docs)
    else:
        # Single query search
        table_query = f"Markdown Table {question}"
        # try:
        table_results = client.similarity_search_with_score(
            table_query, 
            k=10, 
            # filter=metadata_filter
        )
        # except Exception as e:
        #     print(f"Table search error: {e}")
        #     table_results = client.similarity_search_with_score(table_query, k=10)
        
        try:
            text_results = client.similarity_search_with_score(
                question, 
                k=10, 
                # filter=metadata_filter
            )
            ...
            # print(f"Text search error: {e}")
            text_results = client.similarity_search_with_score(question, k=10)

    # Remove duplicates
    unique_table_results = []
    seen_table_content = set()
    for doc, score in table_results:
        if doc.page_content not in seen_table_content:
            unique_table_results.append((doc, score))
            seen_table_content.add(doc.page_content)
    table_results = unique_table_results
    
    unique_text_results = []
    seen_text_content = set()
    for doc, score in text_results:
        if doc.page_content not in seen_text_content:
            unique_text_results.append((doc, score))
            seen_text_content.add(doc.page_content)
    text_results = unique_text_results
    
    # Sort by similarity score (lower is better for ChromaDB)
    table_results.sort(key=lambda x: x[1])
    text_results.sort(key=lambda x: x[1])
    
    # Combine results
    combined_results = table_results[:3] + text_results[:2]
    documents = [doc.page_content for doc, score in combined_results]
...
class RewrittenQueries(BaseModel):
    ...
    year: str = Field(description="Year of the financial document")
    table: str = Field(description="Whether the answer might be in a table")
...
Output exactly 5 rephrased queries for the user question along with the company name and financial year of the document as inferred from question the and nothing else.
...
def possible_queries(state):
    ...
    return {
        "documents": documents, 
        "queries": [question, result.query1, result.query2, result.query3, result.query4, result.query5], 
        "question": question, 
...
if __name__ == "__main__":
    ...
    try:
        ...
        search_results = client.similarity_search_with_score(test_query, k=500)
